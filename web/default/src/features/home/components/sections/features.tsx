  ...
  return (
    <section className='relative z-10 px-6 py-24 md:py-32'>
      <div className='mx-auto max-w-6xl'>
        ...
        {/* Bento grid — floats on the canvas with the soft-shadow system */}
        <div className='border-border/60 bg-border/50 grid gap-px overflow-hidden rounded-2xl border shadow-[var(--soft-shadow-md)] md:grid-cols-3'>
          {features.map((f, i) => (
            <AnimateInView
              key={f.id}
              delay={i * 100}
              animation='scale-in'
              className={`bg-card group hover:bg-muted/20 p-7 transition-colors duration-300 md:p-8 ${f.span}`}
            >
        ...
        <div className='mt-12 grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12'>
          {additionalFeatures.map((f, i) => (
            ...
            >
              <div className='text-primary bg-primary/8 border-primary/15 mb-3 flex size-12 items-center justify-center rounded-xl border transition-colors'>
                {f.icon}
              </div>
